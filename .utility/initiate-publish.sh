...
if [ "$TRAVIS_REPO_SLUG" == "PalomaMobile/android-db-migration" ] && [ "$TRAVIS_PULL_REQUEST" == "false" ] && [ "$TRAVIS_BRANCH" == "master" ]; then
  ...
  if [[ $(gradle -q getVersion) != *SNAPSHOT* ]]; then
    ...
    echo -e "Starting to publish release to Sonatype...\n"
      ...
      gradle uploadArchives -PnexusUsername="${SONATYPE_USERNAME}" -PnexusPassword="${SONATYPE_PASSWORD}" -Psigning.keyId="${SIGN_KEYID}" -Psigning.password="${SIGN_PASSWORD}" -Psigning.secretKeyRingFile=".utility/local.secring.gpg"
      RETVAL=$?
